...
document.addEventListener("mouseover", e => {
  const term = e.target.closest(".wiki-term");
  if (!term) return;
  ...
  tooltip.innerHTML = `
    ...
    <div class="wiki-tooltip-body">
      ${title ? `<div class="wiki-tooltip-title">${title}</div>` : ""}
      ${text ? `<div class="wiki-tooltip-text">${text}</div>` : ""}
    </div>
  `;

  /* tooltip.style.display = "block";*/
  tooltip.style.display = layout === "side" ? "flex" : "block";
});
